...
- **Scope:** whole Lake Constance shoreline (~5 km inclusion band, configurable) — *not* limited to the Bodenseekreis or Landkreis Konstanz. See [docs/product/geographic-scope.md](docs/product/geographic-scope.md).
...
Copy `.env.example` to `.env` before adding local integrations. Values in that file are local defaults or placeholders only; do not commit real secrets.
