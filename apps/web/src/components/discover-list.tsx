  ...
  return (
    <div aria-hidden="true" className="space-y-5">
      {Array.from({ length: 3 }, (_, index) => (
        <div className="grid animate-pulse gap-4 border-b border-slate-800/80 py-5 sm:grid-cols-[12rem_1fr]" key={index}>
...
export function DiscoverList({ initialData, initialError, locale }: DiscoverListProps) {
  ...
  async function loadNextPage() {
    ...
    try {
      const response = await fetch(
        `/api/attractions?locale=${locale}&limit=20&cursor=${encodeURIComponent(nextCursor)}`,
      );
  ...
  if (initialError && items.length === 0) {
    return (
      <section aria-live="polite" className="rounded-lg border border-rose-400/40 bg-rose-950/30 p-6">
  ...
  if (!initialData || items.length === 0) {
    return (
      <section aria-live="polite" className="rounded-lg border border-slate-800 bg-slate-900/60 p-6">
        <h2 className="text-lg font-semibold text-white">{translate('empty.title')}</h2>
        <p className="mt-2 text-sm leading-6 text-slate-400">{translate('empty.description')}</p>
      </section>
    );
  }
  ...
  return (
    <section aria-label={translate('resultsLabel')}>
      ...
      {error ? (
        <div aria-live="polite" className="mt-5 rounded-md border border-rose-400/40 p-4 text-sm text-rose-200">
          <p>{translate('error.description')}</p>
          <button className="mt-3 font-semibold underline" onClick={() => void loadNextPage()} type="button">
            {translate('error.retry')}
          </button>
        </div>
      ) : null}
